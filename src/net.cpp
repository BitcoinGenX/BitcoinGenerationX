...
#if !defined(HAVE_MSG_BGXIGNAL) && !defined(MSG_BGXIGNAL)
#define MSG_BGXIGNAL 0
#endif
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (bitcoingenx users should be well used to small numbers with many decimal places by now :)
...
void SocketSendData(CNode* pnode)
{
    ...
    while (it != pnode->vSendMsg.end()) {
        ...
        int nBytes = send(pnode->hSocket, &data[pnode->nSendOffset], data.size() - pnode->nSendOffset, MSG_BGXIGNAL | MSG_DONTWAIT);
        if (nBytes > 0) {
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "bitcoingenx " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    setsockopt(hListenSocket, SOL_SOCKET, SO_BGXIGPIPE, (void*)&nOne, sizeof(int));
#endif
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BitcoinGenX Core is probably already running."), addrBind.ToString());
        else
