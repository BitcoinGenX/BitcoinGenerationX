...
#include "zbitcoingenxcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBGX ought to be enough for anybody." - Bill Gates, 2017
    ui->zBGXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBGXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBGX->setEnabled(false);
        ui->pushButtonMintzBGX->setToolTip(tr("zBGX is currently disabled due to maintenance."));

        ui->pushButtonSpendzBGX->setEnabled(false);
        ui->pushButtonSpendzBGX->setToolTip(tr("zBGX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBGXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBGX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBGX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBGX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBGX in ") +
...
void PrivacyDialog::on_pushButtonSpendzBGX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBGX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBGX
        sendzBGX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBGX
    sendzBGX();
}

void PrivacyDialog::on_pushButtonZbitcoingenxControl_clicked()
{
    ZbitcoingenxControlDialog* zbitcoingenxControl = new ZbitcoingenxControlDialog(this);
    zbitcoingenxControl->setModel(walletModel);
    zbitcoingenxControl->exec();
}

void PrivacyDialog::setZbitcoingenxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzbitcoingenxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBGX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid bitcoingenx Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBGXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBGX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BGX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBGX</b>";
    ...
    // use mints from zbitcoingenx selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZbitcoingenxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZbitcoingenxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbitcoingenx selector in case it was used
    ZbitcoingenxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zbitcoingenx Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " bitcoingenx, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zbitcoingenx Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBGX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBGX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBGX "));
    ui->labelzBGXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBGXSyncStatus->setVisible(fShow);
}
