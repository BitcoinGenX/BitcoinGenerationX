...
#include "ui_zbitcoingenxcontroldialog.h"
...
std::list<std::string> ZbitcoingenxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZbitcoingenxControlDialog::listMints;

ZbitcoingenxControlDialog::ZbitcoingenxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZbitcoingenxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZbitcoingenxControlDialog::~ZbitcoingenxControlDialog()
{
    delete ui;
}

void ZbitcoingenxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZbitcoingenxControlDialog::updateList()
{
...
void ZbitcoingenxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZbitcoingenxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZbitcoingenx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZbitcoingenxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZbitcoingenxControlDialog::GetSelectedMints()
{
...
void ZbitcoingenxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
