...
void OverviewPage::updateCoinMixingProgress()
{
    ...
    QString strAnonymizeBitcoinGenXAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitcoinGenXAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
      ...
      //  ui->coinmixingProgress->setToolTip(tr("No inputs detected"));
        ...
        strAnonymizeBitcoinGenXAmount = strAnonymizeBitcoinGenXAmount.remove(strAnonymizeBitcoinGenXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitcoinGenXAmount + " / " + tr("%n Rounds", "", nCoinMixingRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBitcoinGenXAmount * COIN) nMaxToAnonymize = nAnonymizeBitcoinGenXAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBitcoinGenXAmount * COIN) {
      //  ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
      //                                        .arg(strAnonymizeBitcoinGenXAmount));
        strAnonymizeBitcoinGenXAmount = strAnonymizeBitcoinGenXAmount.remove(strAnonymizeBitcoinGenXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitcoinGenXAmount + " / " + tr("%n Rounds", "", nCoinMixingRounds);
    } else {
...
void OverviewPage::toggleCoinMixing()
{
    ...
    if (!fEnableCoinMixing) {
      ...
      //  ui->toggleCoinMixing->setText(tr("Stop CoinMixing"));
        ...
        if (nAnonymizeBitcoinGenXAmount == 0) {
