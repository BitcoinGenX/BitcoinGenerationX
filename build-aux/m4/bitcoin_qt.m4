...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; bitcoingenx-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build bitcoingenx-qt GUI (default=auto, qt5 tried first)])],
    [
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test x$use_hardening != xno; then
    BITCOIN_QT_CHECK([
    AC_MSG_CHECKING(whether -fBGX can be used with this Qt config)
    TEMP_CPPFLAGS=$CPPFLAGS
    TEMP_CXXFLAGS=$CXXFLAGS
    CPPFLAGS="$QT_INCLUDES $CPPFLAGS"
    CXXFLAGS="$BGX_FLAGS $CXXFLAGS"
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[#include <QtCore/qconfig.h>]],
      ...
      [ AC_MSG_RESULT(yes); QT_BGX_FLAGS=$BGX_FLAGS ],
      [ AC_MSG_RESULT(no); QT_BGX_FLAGS=$PIC_FLAGS]
    )
    CPPFLAGS=$TEMP_CPPFLAGS
    CXXFLAGS=$TEMP_CXXFLAGS
    ])
    ...
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[#include <QtCore/qconfig.h>]],
      ...
      [ AC_MSG_RESULT(no)],
      [ AC_MSG_RESULT(yes); QT_BGX_FLAGS=$PIC_FLAGS]
    )
    CPPFLAGS=$TEMP_CPPFLAGS
    ])
  fi
  ...
  BITCOIN_QT_PATH_PROGS([LUPDATE], [BGXdate-qt${bitcoin_qt_got_major_vers} BGXdate${bitcoin_qt_got_major_vers} BGXdate],$qt_bin_path, yes)
  ...
  BITCOIN_QT_CHECK([
    ...
    if test x$LUPDATE = x; then
      AC_MSG_WARN("BGXdate is required to update qt translations")
    fi
  ],[
    bitcoin_enable_qt=no
  ])
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Network],[main],,BITCOIN_QT_FAIL(lib$QT_LIB_PREFIBGXetwork not found)))
  if test x$bitcoin_qt_got_major_vers = x5; then
